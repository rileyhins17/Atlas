...
const state = vi.hoisted(() => ({ query: { data: [], isSuccess: true }, focus: vi.fn() }));
...
vi.mock('@/lib/hooks/established', () => ({ useEstablished: () => false }));
...
vi.mock('@/components/stream/FirstCapture', () => ({ FirstCapture: () => null }));
...
beforeEach(() => { state.query.isSuccess = true; state.focus.mockClear(); });
