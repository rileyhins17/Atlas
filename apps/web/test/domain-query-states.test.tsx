...
const state = vi.hoisted(() => ({ pending: false, error: false, retry: vi.fn(), days: vi.fn() }));
...
vi.mock('@/lib/hooks/habits', () => ({
  ...
  useCreateHabit: () => mutation(), useUpdateHabit: () => mutation(), useDeleteHabit: () => mutation(), useLogHabit: () => mutation(),
}));
import { GoalsPanel } from '@/components/panels/GoalsPanel';
import { HabitsPanel } from '@/components/panels/HabitsPanel';
beforeEach(() => { state.pending = false; state.error = false; state.retry.mockClear(); state.days.mockClear(); });
