...
- **A) Google Calendar two-way sync (Riley is getting the credential — start here).** Needs a **Google Cloud OAuth client id + secret** (Calendar API enabled, consent screen External + self as test user, redirect URI `http://localhost:4000/connectors/google/callback`, scope `calendar.events`). Build `packages/connectors/src/google-calendar.ts` per `docs/connector-guide.md`: OAuth start + callback routes, `saveCredential` the tokens (+ refresh handling; store scopes/account email in `meta`), `sync()` two-way against `events` using the existing `source`+`externalId` unique key, writing a `TimelineEvent` per change. Add a Settings screen to connect Google. **Ask Riley the sync-direction question first** — push-only, Google-as-truth, or true two-way (which needs conflict rules). Note `CalendarService.create` currently hardcodes `source: 'atlas'`.
