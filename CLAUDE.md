...
- ✅ **Retrieval wired into chat** (`OrchestratorService.recallText`): embeds the user's message, takes the top 3 matches under a 0.9 distance threshold, appends them to the system prompt. Best-effort — recall failure logs a warning and chat continues. This does what module summaries structurally can't: surface an old entry that's topically relevant but isn't "current state". Verified live: "Have I mentioned feeling any pressure at work recently?" quoted the journal entry back.
...
- **A) Auto-trigger the embedding backfill** (small, unblocked; finishes semantic memory). Right now `MemoryService.queueForEmbedding` writes a `model="pending"` row and nothing ever embeds it unless `POST /ai/embeddings/backfill` is called by hand — so a fresh journal entry isn't recallable. Options: call the backfill after journal/note writes (simple, but adds ~100ms of model time to those requests), or run it on an interval with `@nestjs/schedule` (keeps writes fast; that's the roadmap's Phase 4 scheduler anyway). Embedding is local + free, so there's no cost/cap concern either way.
