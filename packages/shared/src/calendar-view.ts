...
import { addDays, displayDateOptions, localDayKey, startOfDay } from './local-dates.js';
...
export function formatCalendarDuration(mins: number): string {
  if (mins <= 0) return '0m';
...
export function calendarWeekdayShort(d: Date, timezone?: string): string {
  return d.toLocaleDateString('en-US', displayDateOptions({ weekday: 'short' }, timezone));
}

/** "July 2026", or "Jun – Jul 2026" when the visible week straddles months. */
export function calendarRangeLabel(days: Date[], timezone?: string): string {
  if (days.length === 0) return '';
  const first = days[0]!;
  const last = days[days.length - 1]!;
  if (first.getMonth() === last.getMonth() && first.getFullYear() === last.getFullYear()) {
    return first.toLocaleDateString('en-US', displayDateOptions({ month: 'long', year: 'numeric' }, timezone));
  }
  const a = first.toLocaleDateString('en-US', displayDateOptions({ month: 'short' }, timezone));
  const b = last.toLocaleDateString('en-US', displayDateOptions({ month: 'short', year: 'numeric' }, timezone));
  return `${a} – ${b}`;
}
