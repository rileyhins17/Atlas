...
- **The Plaid webhook authenticates the ACK, but still does not trigger a sync.** `POST /connectors/plaid/webhook` is public (Plaid calls it server-to-server), captures Nest's exact `rawBody`, verifies the `Plaid-Verification` ES256 JWT against `/webhook_verification_key/get`, checks its five-minute `iat` window, and compares the signed SHA-256 body hash in constant time. Missing, stale, malformed, or tampered requests get 401; only a verified request gets `{received:true}`. The handler remains acknowledgement-only until webhook-to-item sync is deliberately implemented. It is excluded from browser-origin assumptions because server-to-server requests carry no browser `Origin`.
