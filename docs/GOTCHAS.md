...
- **The AI can start a workout but deliberately cannot log sets or finish one** (`FitnessAiAdapter.tools`). A logged set is a factual claim about what your body did; if the model mishears "three sets of eight", the record is silently wrong and a training log you can't trust is worse than none. Starting a session is harmless and removes the only friction that matters ("I'm at the gym").
