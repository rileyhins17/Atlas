...
| canvas/MoodCheckIn.tsx | useJournal, useRoutine | mood-checkin now positively asserts both loading placeholders, routine failure/Retry with and without an existing answer, and retained failed mood writes. One mutation regression observed red; 20 tests pass. On Today, routine read failure is gated by DayOverview before this child mounts. Browser failed-save/retry and real synthetic journal persistence configured but unverified |
...
| panels/AiSettingsCard.tsx | useAiStatus | ai-access-query-states: eight real-hook cases cover pending/error Retry, approved unavailable, paused hosted, revoked personal account, personal-account invite activation and retained failed invite/retry. Three observed-red issues fixed; eleven focused tests pass including original hook-mocked cases. Presentation browser measurements configured; real authorization/persistence proof remains |
| panels/CalendarPanel.tsx | useEventsRange | core-panel-query-states: actual panel/hooks prove calendar range pending placeholder, failed-read Retry and confirmed-empty day. These cases passed against existing code; connector children are excluded. Both-theme browser failure/empty measurements configured but unverified |
| panels/FinancePanel.tsx | useAccounts, useTransactions | core-panel-query-states: actual panel/hooks prove each source pending and failed-read Retry, confirmed-empty accounts/transactions, and disabled transaction creation while accounts are unknown. These cases passed against existing code; connector children are excluded. Both-theme browser failure/empty measurements configured but unverified |
...
| panels/HabitsPanel.tsx | useHabits, useHabitHistory | domain-query-states: history pending/error/retry preserve check-in controls and hide false blank grids; history request covers displayed 26 weeks; browser recovery passed full and independent runs 34274101707 in both themes; habit-draft-recovery: four observed-red creation/edit pending and retry cases fixed, with duplicate-name behavior retained. New 390px both-theme real-save browser case configured but unverified |
...
| panels/PlaidCard.tsx | usePlaidStatus | bank-panel-recovery: actual pending, failed status/Retry, unconfigured manual alternative, retained failed disconnection and same-item retry, and stale sync result removal. Three observed-red cases fixed; five local tests pass. Provider-intercepted browser measurements configured but unverified |
...
| panels/RoutineEditor.tsx | useRoutine | core-panel-query-states: actual panel/hooks prove pending placeholder, failed-read Retry and confirmed-empty weekly setup. These cases passed against existing code; connector children are excluded. Both-theme browser failure/empty measurements configured but unverified |
...
| panels/WritingPanel.tsx | useJournal, useNotes | core-panel-query-states: actual panel/hooks prove each source pending and failed-read Retry, confirmed-empty writing, and retained confirmed notes during journal failure. These cases passed against existing code; connector children are excluded. Both-theme browser failure/empty measurements configured but unverified |
